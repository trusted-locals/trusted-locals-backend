...
const UNKOWN_ERROR_MESSAGE = 'An unknown error has occured.';
...
async function registerUser(req: Request, res: Response) {
  ...
  if (password.length < 8 || password.length > 64) {
    res.status(400).send({
      errorMessage: 'Password length must be greater than 8 and less than 64'
    });
  }
  ...
  if (name.length > 40 || !/^[a-zA-Z]+$/.test(name)) {
    res.status(400).send({
      errorMessage:
        'Name length should be less than 40 and should contain only alphabets'
    });
  } else {
    try {
      ...
      res
        .status(201)
        .cookie('jwt', token, { maxAge: 86400 * 30 })
      ...
      if (error.errmsg && error.code === 11000) {
        ...
        res
          .status(500)
          .send({ success: false, errorMessage: UNKOWN_ERROR_MESSAGE });
    }
  }
}
  ...
  try {
    ...
    if (!user) {
      return res.status(401).send({
        success: false,
        errorMessage: AUTHENTICATION_FAILED_ERROR_MESSAGE
      });
    }
    ...
    console.log(error);
    res.status(500).send({ errorMessage: UNKOWN_ERROR_MESSAGE });
  }
}
